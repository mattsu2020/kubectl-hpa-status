...
func TestAnalyzeBatchAppliesEnrichmentWarningsAndPreservesOrder(t *testing.T) {
	...
	if results[0].Analysis.Controllers.KEDAInfo == nil {
		t.Fatal("first result is missing KEDA enrichment")
	}
	if len(results[0].Analysis.Actions.Warnings) != 1 || results[0].Analysis.Actions.Warnings[0] != "KEDA list unavailable" {
		t.Fatalf("first warnings = %#v", results[0].Analysis.Actions.Warnings)
	}
	if results[1].Analysis.Advisory.VPAConflict == nil {
...
func TestAnalyzeBatchDoesNotAliasNamespaceWarnings(t *testing.T) {
	...
	warnings[0] = "mutated"
	if got := results[0].Analysis.Actions.Warnings[0]; got != "unavailable" {
