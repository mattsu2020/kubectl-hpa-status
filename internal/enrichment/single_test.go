...
func TestEnrichVPA_NoConflict(t *testing.T) {
	...
	if report.Analysis.Advisory.VPAConflict != nil {
		t.Errorf("expected no VPA conflict recorded, got %+v", report.Analysis.Advisory.VPAConflict)
	}
}
...
func TestEnrichReport_FullPath(t *testing.T) {
	...
	if report.Analysis.Lifecycle.EnrichmentStatus.KEDA == nil || report.Analysis.Lifecycle.EnrichmentStatus.VPA == nil {
		t.Errorf("expected enrichment status for both KEDA and VPA, got %+v", report.Analysis.Lifecycle.EnrichmentStatus)
	}
}
