...
func TestE2E_ConfigFile(t *testing.T) {
	...
	defaultCmd.SetArgs([]string{"status", "config-hpa", "-n", nsName, "-o", "json", "--output-schema=v2", "--kubeconfig", kubeconfig})
...
`
	...
	configCmd.SetArgs([]string{"status", "config-hpa", "-n", nsName, "-o", "json", "--output-schema=v2", "--config", configPath, "--kubeconfig", kubeconfig})
