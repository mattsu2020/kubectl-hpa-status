...
func TestE2E_MaxReplicasCap(t *testing.T) {
	...
	if a.Decision.Health != "LIMITED" {
		t.Errorf("expected Health=LIMITED at maxReplicas, got %q (score=%d)", a.Decision.Health, a.Decision.HealthScore)
	}

	// Summary must announce the maxReplicas cap.
	if !strings.Contains(a.Decision.Summary, "maxReplicas") {
		t.Errorf("expected Summary to mention maxReplicas, got %q", a.Decision.Summary)
	}

	// ScalingLimited condition should be present once the controller reports it.
	if !conditionPresent(a.Conditions.Conditions, "ScalingLimited") {
		t.Logf("Note: ScalingLimited condition not present yet (controller may still be converging); summary=%q", a.Decision.Summary)
	}
}
