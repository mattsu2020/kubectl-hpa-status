...
// defaultLabelValues is the English source of truth for labels consumed by
// pkg/hpa renderers. Keep this as package data rather than rebuilding the map
// on every lookup; synchronization with the embedded locales is enforced by
// TestResolvedLabelCatalogIsSynchronized.
...
func (DefaultLabels) Get(key string) string {
	if v, ok := defaultLabelValues[key]; ok {
		return v
	}
	return key
}
