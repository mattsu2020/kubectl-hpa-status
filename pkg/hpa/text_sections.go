...
func appendScoreBreakdown(out *[]byte, a *Analysis) {
	if a.Decision.HealthResult == nil || len(a.Decision.HealthResult.Signals) == 0 {
		return
	}
...
func appendStabilizationAndBehavior(out *[]byte, a *Analysis, theme style.Theme, labels labels) {
	...
	if len(a.Conditions.Behavior) > 0 {
		*out = append(*out, '\n')
		*out = fmt.Appendf(*out, "%s:\n", labels.Behavior)
		for _, behavior := range a.Conditions.Behavior {
...
func appendHiddenFactors(out *[]byte, a *Analysis, opts StatusTextOptions) {
	if !opts.HiddenFactors || len(a.Lifecycle.HiddenFactors) == 0 {
		return
	}
	*out = append(*out, '\n')
	...
	for _, suggestion := range a.Actions.Suggestions {
		...
		*out = append(*out, '\n')
		...
		if opts.Diff && suggestion.Patch != "" {
			currentMin := a.Replicas.Min
			*out = fmt.Appendf(*out, "    diff:\n%s", indentBlock(SuggestionDiff(&currentMin, a.Replicas.Desired, a.Replicas.Max, suggestion.Patch), "      "))
		}
...
func appendKEDASection(out *[]byte, a *Analysis, theme style.Theme, labels labels) {
	...
	*out = append(*out, '\n')
	*out = fmt.Appendf(*out, "%s:\n", labels.KEDA)
	*out = fmt.Appendf(*out, "  ScaledObject: %s\n", a.Controllers.KEDAInfo.ScaledObjectName)
	...
	if a.Controllers.KEDAInfo.MinReplicaCount != nil {
		*out = fmt.Appendf(*out, "  Min replica count: %d\n", *a.Controllers.KEDAInfo.MinReplicaCount)
	}
	if a.Controllers.KEDAInfo.MaxReplicaCount != nil {
		*out = fmt.Appendf(*out, "  Max replica count: %d\n", *a.Controllers.KEDAInfo.MaxReplicaCount)
	}
	for _, line := range a.Controllers.KEDAInfo.Lines {
...
func appendVPASection(out *[]byte, a *Analysis, theme style.Theme) {
	...
	*out = fmt.Appendf(*out, "  %s targets %s/%s", a.Advisory.VPAConflict.VPAName, a.Advisory.VPAConflict.TargetKind, a.Advisory.VPAConflict.TargetName)
	if a.Advisory.VPAConflict.UpdateMode != "" {
		*out = fmt.Appendf(*out, " updateMode=%s", a.Advisory.VPAConflict.UpdateMode)
	}
	*out = append(*out, '\n')
	if len(a.Advisory.VPAConflict.ControlledResources) > 0 {
		*out = fmt.Appendf(*out, "  Controlled resources: %s\n", strings.Join(a.Advisory.VPAConflict.ControlledResources, ", "))
	}
	...
	*out = fmt.Appendf(*out, "  warning: %s\n", theme.ActionLine(a.Advisory.VPAConflict.Warning))
}
...
func appendMetricsDiagnosticsSection(out *[]byte, a *Analysis, theme style.Theme, labels labels) {
	...
	*out = fmt.Appendf(*out, "  %s: %s\n", labels.Health, metricsDiagnosticsStatus(a.Metrics.MetricsDiagnostics.OverallStatus, theme))
	if len(a.Metrics.MetricsDiagnostics.PerMetricChecks) > 0 {
...
func appendResourceCheckSection(out *[]byte, a *Analysis) {
	if a.Capacity.ResourceCheck == nil || len(a.Capacity.ResourceCheck.Warnings) == 0 {
		return
	}
	*out = append(*out, '\n')
	*out = append(*out, "Resource Check:\n"...)
	for _, w := range a.Capacity.ResourceCheck.Warnings {
		indicator := "warning"
		if w.Severity == "error" {
			indicator = "error"
		}
