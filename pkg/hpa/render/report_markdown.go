// Package render holds the Markdown/HTML report renderers extracted from
// pkg/hpa. The renderers consume the pkg/hpa analysis model (Analysis,
// StatusReport, ListReport) and the shared escape/format helpers in
// pkg/hpa/rendutil. pkg/hpa retains the data model and analysis logic; this
// package is the pure presentation layer.
package render

import (
	"fmt"
	"io"
	"strings"

	hpa "github.com/mattsu2020/kubectl-hpa-status/pkg/hpa"
	"github.com/mattsu2020/kubectl-hpa-status/pkg/hpa/rendutil"
)

// WriteMarkdownReport writes a single StatusReport as a Markdown document.
// The output is suitable for Slack, Notion, or incident reports.
func WriteMarkdownReport(w io.Writer, report hpa.StatusReport) error {
	var out strings.Builder
	// Pass Analysis by pointer: 11 append* helpers each previously copied the
	// 63-field Analysis struct.
	a := &report.Analysis
	appendMarkdownHeader(&out, a)
	for _, section := range reportSections[1:] { // overview is included in the Markdown header
		appendMarkdownSection(&out, report, section)
	}
	out.WriteString("---\nGenerated by kubectl-hpa-status\n")

	_, err := io.WriteString(w, out.String())
	return err
}

func appendMarkdownSection(out *strings.Builder, report hpa.StatusReport, section reportSection) {
	a := &report.Analysis
	switch section {
	case sectionSummary:
		appendMarkdownSummary(out, a)
	case sectionConditions:
		appendMarkdownConditions(out, a)
	case sectionMetrics:
		appendMarkdownMetrics(out, a)
	case sectionRecommendations:
		appendMarkdownActions(out, a)
	case sectionSuggestions:
		appendMarkdownSuggestions(out, a)
	case sectionEvents:
		appendMarkdownEvents(out, report)
	case sectionPods:
		appendMarkdownPodAnalysis(out, a)
	case sectionSimulation:
		appendMarkdownSimulation(out, a)
	case sectionMetricFreshness:
		appendMarkdownMetricFreshness(out, a)
	case sectionCapacity:
		appendMarkdownCapacityContext(out, a)
	case sectionDecisionTrace:
		appendMarkdownStructuredDecisionTrace(out, a)
	case sectionWarnings:
		appendMarkdownWarnings(out, a)
	}
}

func appendMarkdownHeader(out *strings.Builder, a *hpa.Analysis) {
	out.WriteString("# HPA Status Report: ")
	out.WriteString(rendutil.MarkdownInline(a.Meta.Name))
	if a.Meta.Namespace != "" {
		out.WriteString(" (")
		out.WriteString(rendutil.MarkdownInline(a.Meta.Namespace))
		out.WriteString(")")
	}
	out.WriteString("\n\n")

	out.WriteString("**Target:** ")
	out.WriteString(rendutil.MarkdownInline(a.Meta.Target))
	out.WriteString("\n")

	out.WriteString("**Health:** ")
	out.WriteString(a.Decision.Health)
	out.WriteString(fmt.Sprintf(" (%d/100)", a.Decision.HealthScore))
	out.WriteString("\n")

	out.WriteString(fmt.Sprintf("**Replicas:** current=%d desired=%d min=%d max=%d\n", a.Replicas.Current, a.Replicas.Desired, a.Replicas.Min, a.Replicas.Max))
	out.WriteString("\n")
}

func appendMarkdownSummary(out *strings.Builder, a *hpa.Analysis) {
	out.WriteString("## Summary\n\n")
	if a.Decision.Summary != "" {
		out.WriteString(rendutil.MarkdownInline(a.Decision.Summary))
		out.WriteString("\n")
	} else {
		out.WriteString("_No summary available._\n")
	}
	out.WriteString("\n")
}

func appendMarkdownConditions(out *strings.Builder, a *hpa.Analysis) {
	// Conditions table
	out.WriteString("## Conditions\n\n")
	if len(a.Conditions.Conditions) == 0 {
		out.WriteString("_No conditions reported._\n")
	} else {
		out.WriteString("| Type | Status | Reason | Message |\n")
		out.WriteString("|------|--------|--------|--------|\n")
		for _, c := range a.Conditions.Conditions {
			out.WriteString("| ")
			out.WriteString(rendutil.MarkdownCell(c.Type))
			out.WriteString(" | ")
			out.WriteString(rendutil.MarkdownCell(c.Status))
			out.WriteString(" | ")
			out.WriteString(rendutil.MarkdownCell(c.Reason))
			out.WriteString(" | ")
			out.WriteString(rendutil.MarkdownCell(c.Message))
			out.WriteString(" |\n")
		}
	}
	out.WriteString("\n")
}

func appendMarkdownMetrics(out *strings.Builder, a *hpa.Analysis) {
	// Metrics table
	out.WriteString("## Metrics\n\n")
	if len(a.Metrics.Metrics) == 0 {
		out.WriteString("_No current metrics reported._\n")
	} else {
		out.WriteString("| Metric | Current | Target | Ratio |\n")
		out.WriteString("|--------|---------|--------|-------|\n")
		for _, m := range a.Metrics.Metrics {
			name := m.Name
			if name == "" {
				name = m.Type
			}
			ratio := ""
			if m.Ratio != nil {
				ratio = fmt.Sprintf("%.3f", *m.Ratio)
			}
			out.WriteString("| ")
			out.WriteString(rendutil.MarkdownCell(name))
			out.WriteString(" | ")
			out.WriteString(rendutil.MarkdownCell(m.Current))
			out.WriteString(" | ")
			out.WriteString(rendutil.MarkdownCell(m.Target))
			out.WriteString(" | ")
			out.WriteString(ratio)
			out.WriteString(" |\n")
		}
	}
	out.WriteString("\n")
}

func appendMarkdownActions(out *strings.Builder, a *hpa.Analysis) {
	// Recommendations
	if len(a.Actions.Actions) == 0 {
		return
	}
	out.WriteString("## Recommendations\n\n")
	for _, action := range a.Actions.Actions {
		out.WriteString("- ")
		out.WriteString(rendutil.MarkdownInline(action))
		out.WriteString("\n")
	}
	out.WriteString("\n")
}

func appendMarkdownSuggestions(out *strings.Builder, a *hpa.Analysis) {
	// Suggestions
	if len(a.Actions.Suggestions) == 0 {
		return
	}
	out.WriteString("## Suggestions\n\n")
	for _, s := range a.Actions.Suggestions {
		out.WriteString("- **")
		out.WriteString(rendutil.MarkdownInline(s.Title))
		out.WriteString("**: ")
		out.WriteString(rendutil.MarkdownInline(s.Description))
		out.WriteString("\n")
		if s.Command != "" {
			out.WriteString("  ```\n  ")
			out.WriteString(s.Command)
			out.WriteString("\n  ```\n")
		}
		if s.Risk != "" {
			out.WriteString("  - Risk: ")
			out.WriteString(s.Risk)
			out.WriteString("\n")
		}
	}
	out.WriteString("\n")
}

func appendMarkdownEvents(out *strings.Builder, report hpa.StatusReport) {
	// Events
	if len(report.Events) == 0 {
		return
	}
	out.WriteString("## Events\n\n")
	out.WriteString("| Reason | Message |\n")
	out.WriteString("|--------|--------|\n")
	for _, e := range report.Events {
		out.WriteString("| ")
		out.WriteString(rendutil.MarkdownCell(e.Reason))
		out.WriteString(" | ")
		out.WriteString(rendutil.MarkdownCell(e.Message))
		out.WriteString(" |\n")
	}
	out.WriteString("\n")
}

func appendMarkdownPodAnalysis(out *strings.Builder, a *hpa.Analysis) {
	// Pod Analysis
	if a.Capacity.PodAnalysis == nil {
		return
	}
	pa := a.Capacity.PodAnalysis
	out.WriteString("## Pod Analysis\n\n")
	out.WriteString(fmt.Sprintf("- **Total:** %d  **Ready:** %d  **Unready:** %d  **Pending:** %d  **Terminating:** %d\n", pa.Total, pa.Ready, pa.Unready, pa.Pending, pa.Terminating))
	if len(pa.ResourceIssues) > 0 {
		out.WriteString("\n### Missing Resources\n\n")
		out.WriteString("| Pod | Container | Resource | Category |\n")
		out.WriteString("|-----|-----------|----------|----------|\n")
		for _, issue := range pa.ResourceIssues {
			out.WriteString(fmt.Sprintf("| %s | %s | %s | %s |\n", rendutil.EscapeMarkdown(issue.Pod), rendutil.EscapeMarkdown(issue.Container), rendutil.EscapeMarkdown(issue.Resource), rendutil.EscapeMarkdown(issue.Category)))
		}
		out.WriteString("\n")
	}
	if len(pa.ContainerChecks) > 0 {
		out.WriteString("\n### Container Checks\n\n")
		out.WriteString("| Container | Found | Message |\n")
		out.WriteString("|-----------|-------|--------|\n")
		for _, check := range pa.ContainerChecks {
			msg := check.Message
			if msg == "" {
				msg = "OK"
			}
			out.WriteString(fmt.Sprintf("| %s | %v | %s |\n", rendutil.EscapeMarkdown(check.Container), check.Found, rendutil.EscapeMarkdown(msg)))
		}
		out.WriteString("\n")
	}
}

func appendMarkdownSimulation(out *strings.Builder, a *hpa.Analysis) {
	// Simulation
	if a.Stability.FlappingSimulation == nil {
		return
	}
	sim := a.Stability.FlappingSimulation
	out.WriteString("## Simulation\n\n")
	out.WriteString(fmt.Sprintf("- **Parameter:** %s\n", rendutil.MarkdownInline(sim.Parameter)))
	out.WriteString(fmt.Sprintf("- **Original:** %s  **Simulated:** %s\n", rendutil.MarkdownInline(sim.OriginalValue), rendutil.MarkdownInline(sim.SimulatedValue)))
	out.WriteString(fmt.Sprintf("- **Before:** desired=%d health=%s(%d)  **After:** desired=%d health=%s(%d)\n",
		sim.Before.DesiredReplicas, rendutil.MarkdownInline(sim.Before.Health), sim.Before.HealthScore,
		sim.After.DesiredReplicas, rendutil.MarkdownInline(sim.After.Health), sim.After.HealthScore))
	if sim.RiskAssessment != "" {
		out.WriteString(fmt.Sprintf("- **Risk:** %s\n", rendutil.MarkdownInline(sim.RiskAssessment)))
	}
	if len(sim.Interpretation) > 0 {
		out.WriteString("\n")
		for _, line := range sim.Interpretation {
			out.WriteString(fmt.Sprintf("- %s\n", rendutil.MarkdownInline(line)))
		}
	}
	out.WriteString("\n")
}

func appendMarkdownMetricFreshness(out *strings.Builder, a *hpa.Analysis) {
	// Metrics Freshness
	if len(a.Metrics.MetricFreshness) == 0 {
		return
	}
	out.WriteString("## Metrics Freshness\n\n")
	for _, mf := range a.Metrics.MetricFreshness {
		out.WriteString(fmt.Sprintf("### %s (%s) — %s\n\n", rendutil.MarkdownInline(mf.Name), rendutil.MarkdownInline(mf.Type), rendutil.MarkdownInline(mf.Status)))
		if mf.Source != "" {
			out.WriteString(fmt.Sprintf("- **Source:** %s\n", rendutil.MarkdownInline(mf.Source)))
		}
		if mf.Window != "" {
			out.WriteString(fmt.Sprintf("- **Window:** %s\n", rendutil.MarkdownInline(mf.Window)))
		}
		if mf.Risk != "" {
			out.WriteString(fmt.Sprintf("- **Risk:** %s\n", rendutil.MarkdownInline(mf.Risk)))
		}
		if len(mf.Evidence) > 0 {
			out.WriteString("- **Evidence:**\n")
			for _, e := range mf.Evidence {
				out.WriteString(fmt.Sprintf("  - %s\n", rendutil.MarkdownInline(e)))
			}
		}
		if len(mf.NextSteps) > 0 {
			out.WriteString("- **Next Steps:**\n")
			for _, ns := range mf.NextSteps {
				out.WriteString(fmt.Sprintf("  - %s\n", rendutil.MarkdownInline(ns)))
			}
		}
		out.WriteString("\n")
	}
}

func appendMarkdownCapacityContext(out *strings.Builder, a *hpa.Analysis) {
	// Capacity Context
	cc := a.Capacity.CapacityContext
	if cc == nil {
		return
	}
	tables := capacityContextTables(cc)
	if len(tables) == 0 && len(cc.NodeHints) == 0 {
		return
	}
	out.WriteString("## Capacity Context\n\n")
	for _, t := range tables {
		writeMarkdownTable(out, t)
	}
	if len(cc.NodeHints) > 0 {
		out.WriteString("### Hints\n\n")
		for _, hint := range cc.NodeHints {
			out.WriteString(fmt.Sprintf("- %s\n", rendutil.MarkdownInline(hint)))
		}
		out.WriteString("\n")
	}
}

func appendMarkdownStructuredDecisionTrace(out *strings.Builder, a *hpa.Analysis) {
	// Structured Decision Trace
	if a.Decision.StructuredDecisionTrace == nil {
		return
	}
	sdt := a.Decision.StructuredDecisionTrace
	out.WriteString("## Structured Decision Trace\n\n")
	out.WriteString(fmt.Sprintf("- **Schema:** %s\n", rendutil.MarkdownInline(sdt.SchemaVersion)))
	out.WriteString(fmt.Sprintf("- **HPA:** %s/%s\n", rendutil.MarkdownInline(sdt.Namespace), rendutil.MarkdownInline(sdt.Name)))
	out.WriteString(fmt.Sprintf("- **Replicas:** current=%d desired=%d min=%d max=%d\n",
		sdt.CurrentReplicas, sdt.VisibleDesiredReplicas, sdt.MinReplicas, sdt.MaxReplicas))
	if sdt.EstimatedRawDesired > 0 {
		out.WriteString(fmt.Sprintf("- **Estimated raw desired:** %d\n", sdt.EstimatedRawDesired))
	}
	if sdt.WinnerMetric != "" {
		out.WriteString(fmt.Sprintf("- **Winner metric:** %s (confidence: %s)\n", rendutil.MarkdownInline(sdt.WinnerMetric), rendutil.MarkdownInline(string(sdt.WinnerConfidence))))
	}
	if sdt.LimitClamp != "" {
		out.WriteString(fmt.Sprintf("- **Limit clamp:** %s\n", rendutil.MarkdownInline(sdt.LimitClamp)))
	}
	if sdt.Summary != "" {
		out.WriteString(fmt.Sprintf("- **Summary:** %s\n", rendutil.MarkdownInline(sdt.Summary)))
	}
	if len(sdt.Metrics) > 0 {
		out.WriteString("\n### Metric Analysis\n\n")
		out.WriteString("| Metric | Type | Current | Target | Ratio | Direction | Within Tolerance |\n")
		out.WriteString("|--------|------|---------|--------|-------|-----------|------------------|\n")
		for _, m := range sdt.Metrics {
			ratio := ""
			if m.Ratio != nil {
				ratio = fmt.Sprintf("%.3f", *m.Ratio)
			}
			tolerance := "no"
			if m.WithinTolerance {
				tolerance = "yes"
			}
			out.WriteString(fmt.Sprintf("| %s | %s | %s | %s | %s | %s | %s |\n",
				rendutil.MarkdownCell(m.Name), rendutil.MarkdownCell(m.Type), rendutil.MarkdownCell(m.Current),
				rendutil.MarkdownCell(m.Target), ratio, rendutil.MarkdownCell(m.DesiredDirection), tolerance))
		}
		out.WriteString("\n")
	}
	if len(sdt.DecisionPath) > 0 {
		out.WriteString("### Decision Path\n\n")
		out.WriteString("| Step | Description | Result | Impact | Confidence |\n")
		out.WriteString("|------|-------------|--------|--------|------------|\n")
		for _, step := range sdt.DecisionPath {
			out.WriteString(fmt.Sprintf("| %d | %s | %s | %s | %s |\n",
				step.Step, rendutil.MarkdownCell(step.Description), rendutil.MarkdownCell(step.Result),
				rendutil.MarkdownCell(step.Impact), rendutil.MarkdownCell(string(step.Confidence))))
		}
		out.WriteString("\n")
	}
}

func appendMarkdownWarnings(out *strings.Builder, a *hpa.Analysis) {
	if len(a.Actions.Warnings) == 0 {
		return
	}
	out.WriteString("## Warnings\n\n")
	for _, warning := range a.Actions.Warnings {
		out.WriteString("- ")
		out.WriteString(rendutil.MarkdownInline(warning))
		out.WriteByte('\n')
	}
	out.WriteByte('\n')
}
