// Package metricidentity holds the pure metric identity and status helpers
// shared by the hpa root package and the simulate package. Keeping them in a
// lower-level package lets both call the same code directly instead of
// routing through function-pointer injection from the hpa root's init().
package metricidentity
...
// CanonicalMetricSelector renders a label selector in its canonical string
// form; nil and empty selectors canonicalize to the empty string.
...
// IdentityMatches reports whether a spec metric and a current status entry
// describe the same metric.
func IdentityMatches(spec autoscalingv2.MetricSpec, current autoscalingv2.MetricStatus) bool {
...
// status, reporting false for missing or malformed entries.
...
// MatchingTarget returns the spec target that belongs to the current status
// entry, matched by canonical identity.
func MatchingTarget(hpa *autoscalingv2.HorizontalPodAutoscaler, current autoscalingv2.MetricStatus) (*autoscalingv2.MetricTarget, bool) {
	for i := range hpa.Spec.Metrics {
		spec := &hpa.Spec.Metrics[i]
		if IdentityMatches(*spec, current) {
			descriptor, err := MetricDescriptorFromSpec(*spec)
...
// HasValueForTarget reports whether the status value payload carries the
// field the given target type reads.
func HasValueForTarget(v autoscalingv2.MetricValueStatus, targetType autoscalingv2.MetricTargetType) bool {
	switch targetType {
