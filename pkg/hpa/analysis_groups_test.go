...
func TestAnalysisGroupViews(t *testing.T) {
	a := &Analysis{
		Meta:       MetaView{Namespace: "production", Name: "web", Target: "Deployment/web"},
		Replicas:   ReplicasView{Current: 5, Desired: 7, Min: 2, Max: 10},
		Decision:   DecisionView{Health: "OK", HealthScore: 90, Summary: "scaling up", SummaryKey: "dir_scale_up"},
		Metrics:    MetricsView{Metrics: []Metric{{Name: "cpu"}}},
		Conditions: ConditionsView{Conditions: []Condition{{Type: "ScalingActive", Status: "True"}}},
		Actions:    ActionsView{Actions: []string{"raise maxReplicas"}, Warnings: []string{"enrichment skipped"}},
	}
...
func TestAnalysisGroupViews_SharedPointersAreReadOnly(t *testing.T) {
	...
	a := &Analysis{
		Replicas: ReplicasView{TargetReplicas: tr},
		Capacity: CapacityView{CapacityPlan: plan},
	}
