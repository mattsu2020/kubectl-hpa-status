...
type ChurnAnalysis struct {
	...
	// Level is the qualitative churn classification based on Score.
...
// rescaleData is an internal struct holding a parsed rescale event's timestamp
// and the new replica count.
...
// AnalyzeChurnFromSnapshots detects thrashing/churn patterns in HPA scaling by
...
func buildChurnAnalysis(rescales []rescaleData, hpa *autoscalingv2.HorizontalPodAutoscaler) *ChurnAnalysis {
	scaleUpCount := 0
	scaleDownCount := 0
	directionFlips := 0
...
// parsed. This is a local duplicate of parseNewSize to avoid cross-file
// coupling with the retrospective module.
