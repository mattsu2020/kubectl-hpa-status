...
func staleStatusRule(hpa *autoscalingv2.HorizontalPodAutoscaler, _ int32, ctx *diagnosticContext) {
	...
	ctx.cases = append(ctx.cases, interpretationCase{
		reason:     "StaleStatus",
		message:    fmt.Sprintf(confidence.BadgeObserved+" Warning: status.observedGeneration=%d is behind metadata.generation=%d; the status may not reflect the latest spec.", *hpa.Status.ObservedGeneration, hpa.Generation),
...
func scalingActiveDiagnosticRule(hpa *autoscalingv2.HorizontalPodAutoscaler, _ int32, ctx *diagnosticContext) {
	...
	ctx.cases = append(ctx.cases,
		interpretationCase{
			...
			message:    fmt.Sprintf(confidence.BadgeObserved+" ScalingActive is %s: %s - %s", condition.Status, condition.Reason, condition.Message),
		...
		interpretationCase{
			reason:     "ScalingInactive",
			message:    confidence.BadgeObserved + " The HPA is not reporting a reliable scale direction while metric evaluation is inactive.",
		...
		interpretationCase{
			reason:     "ScalingInactive",
			message:    confidence.BadgeObserved + " This plugin avoids treating desiredReplicas=0 as a scale-down recommendation in this state.",
...
func ableToScaleDiagnosticRule(hpa *autoscalingv2.HorizontalPodAutoscaler, _ int32, ctx *diagnosticContext) {
	...
	if condition.Status != corev1.ConditionTrue {
		ctx.cases = append(ctx.cases, interpretationCase{
			...
			message:    fmt.Sprintf(confidence.BadgeObserved+" AbleToScale is %s: %s - %s", condition.Status, condition.Reason, condition.Message),
	...
	if condition.Reason != ReasonScaleDownStabilized {
		return
	}
	if remaining := estimateStabilizationRemaining(hpa); remaining != nil && *remaining > 0 {
		message := fmt.Sprintf(confidence.BadgeEstimated+" Scale down appears stabilized: %s (estimated ~%d seconds remaining before scale-down is allowed).", condition.Message, *remaining)
	...
	ctx.cases = append(ctx.cases, interpretationCase{
		reason:     ReasonScaleDownStabilized,
		message:    fmt.Sprintf(confidence.BadgeEstimated+" Scale down appears stabilized: %s", condition.Message),
		severity:   SeverityInfo,
		confidence: ConfidenceMedium,
	})
}
...
func scalingLimitedDiagnosticRule(hpa *autoscalingv2.HorizontalPodAutoscaler, minReplicas int32, ctx *diagnosticContext) {
	...
	case hpa.Spec.MaxReplicas:
		ctx.cases = append(ctx.cases, interpretationCase{
			...
			message:    confidence.BadgeObserved + " ScalingLimited reports that the visible desired replica count is constrained by maxReplicas.",
	...
	case minReplicas:
		ctx.cases = append(ctx.cases, interpretationCase{
			...
			message:    confidence.BadgeObserved + " ScalingLimited reports that the visible desired replica count is constrained by minReplicas.",
	...
	default:
		ctx.cases = append(ctx.cases, interpretationCase{
			...
			message:    confidence.BadgeObserved + " The recommendation is reported as limited.",
			severity:   SeverityWarning,
			confidence: ConfidenceHigh,
		})
	}
...
func replicaDirectionRule(hpa *autoscalingv2.HorizontalPodAutoscaler, minReplicas int32, ctx *diagnosticContext) {
	...
	case hpa.Status.DesiredReplicas > hpa.Status.CurrentReplicas:
		ctx.cases = append(ctx.cases, interpretationCase{
			...
			message:    confidence.BadgeObserved + " desiredReplicas is greater than currentReplicas, so the HPA is recommending scale up.",
			severity:   SeverityInfo,
			confidence: ConfidenceHigh,
		})
	case hpa.Status.DesiredReplicas < hpa.Status.CurrentReplicas:
		ctx.cases = append(ctx.cases, interpretationCase{
			...
			message:    confidence.BadgeObserved + " desiredReplicas is less than currentReplicas, so the HPA is recommending scale down.",
			severity:   SeverityInfo,
			confidence: ConfidenceHigh,
		})
	default:
		ctx.cases = append(ctx.cases, interpretationCase{
			...
			message:    confidence.BadgeObserved + " desiredReplicas equals currentReplicas, so no immediate replica change is visible from status.",
			severity:   SeverityInfo,
			confidence: ConfidenceHigh,
		})
...
func toleranceDiagnosticRule(hpa *autoscalingv2.HorizontalPodAutoscaler, minReplicas int32, ctx *diagnosticContext) {
	...
	if withinTolerance {
		...
		claim := confidence.BadgeEstimated
		if condition := FindCondition(hpa, ConditionAbleToScale); condition != nil && condition.Reason == "DesiredWithinTolerance" {
			...
			claim = "[tolerance-confirmed] " + confidence.BadgeObserved
		}
		ctx.cases = append(ctx.cases, interpretationCase{
		...
		ctx.cases = append(ctx.cases,
			interpretationCase{
				reason:     "ToleranceNoScale",
				message:    fmt.Sprintf(confidence.BadgeEstimated+" %s metric ratio is approximately %.3f, which is close to the target.", metric.Name, metric.Ratio),
				severity:   SeverityInfo,
				confidence: ConfidenceMedium,
			},
			interpretationCase{
				reason:     "ToleranceNoScale",
				message:    confidence.BadgeEstimated + " This is consistent with tolerance-based no-scale. Kubernetes commonly uses a tolerance band around the target, but HPA status does not expose tolerance as an explicit reason.",
				severity:   SeverityInfo,
				confidence: ConfidenceMedium,
			},
		)
	...
	ctx.cases = append(ctx.cases, interpretationCase{
		reason:     "ToleranceNoScale",
		message:    confidence.BadgeObserved + " The plugin avoids claiming the exact internal reason because rounding, stabilization, or conservative metric handling may also affect the final result.",
		severity:   SeverityInfo,
		confidence: ConfidenceHigh,
	})
}
...
func multiMetricRule(hpa *autoscalingv2.HorizontalPodAutoscaler, _ int32, ctx *diagnosticContext) {
	if hpa.Status.DesiredReplicas == hpa.Spec.MaxReplicas && len(hpa.Status.CurrentMetrics) > 1 {
		ctx.cases = append(ctx.cases, interpretationCase{
			...
			message:    confidence.BadgeObserved + " desiredReplicas == maxReplicas; the winning metric cannot be reliably determined because the replica cap may hide the true metric winner.",
			severity:   SeverityInfo,
			confidence: ConfidenceHigh,
		})
		return
	...
	if guess, ok := MostInfluentialMetric(hpa); ok && len(hpa.Status.CurrentMetrics) > 1 {
		ctx.cases = append(ctx.cases,
			interpretationCase{
				...
				message:    fmt.Sprintf(confidence.BadgeEstimated+" Among visible metrics, %s has the largest distance from target (ratio %.3f).", guess.Name, guess.Ratio),
				severity:   SeverityInfo,
				confidence: ConfidenceMedium,
			},
			interpretationCase{
				reason:     "MetricImpactEstimate",
				message:    confidence.BadgeObserved + " This is only an impact estimate; the API does not expose per-metric replica recommendations or the final metric winner.",
				severity:   SeverityInfo,
				confidence: ConfidenceHigh,
			},
		)
	...
	if len(hpa.Status.CurrentMetrics) > 1 {
		ctx.cases = append(ctx.cases,
			interpretationCase{
				...
				message:    confidence.BadgeObserved + " Multiple current metrics are reported, but the API does not expose per-metric replica recommendations or which metric would have selected the recommendation before replica limits were applied.",
				severity:   SeverityInfo,
				confidence: ConfidenceHigh,
			},
			interpretationCase{
				reason:     "MultiMetricNoWinner",
				message:    confidence.BadgeObserved + " Events and human-readable messages can hint at the contributing metric, but they are not a stable decision record.",
				severity:   SeverityInfo,
				confidence: ConfidenceHigh,
			},
		)
...
func metricDisagreementRule(hpa *autoscalingv2.HorizontalPodAutoscaler, _ int32, ctx *diagnosticContext) {
	...
	if len(scaleUp) > 0 && len(scaleDown) > 0 {
		ctx.cases = append(ctx.cases, interpretationCase{
			...
			message:    fmt.Sprintf(confidence.BadgeEstimated+" Metric disagreement detected: %s want scale-up (ratio > 1.0) while %s want scale-down (ratio < 1.0). The HPA controller will use its selectPolicy to resolve this, but consider whether the metric targets are well-tuned.", strings.Join(scaleUp, ", "), strings.Join(scaleDown, ", ")),
			severity:   SeverityWarning,
			confidence: ConfidenceMedium,
		})
	}
...
func scaleToZeroRule(hpa *autoscalingv2.HorizontalPodAutoscaler, minReplicas int32, ctx *diagnosticContext) {
	...
	if hpa.Status.DesiredReplicas == 0 && hpa.Status.CurrentReplicas == 0 {
		ctx.cases = append(ctx.cases, interpretationCase{
			reason:     "ScaleToZero",
			message:    confidence.BadgeObserved + " Scale-to-zero is enabled (minReplicas=0) and the workload is currently at zero replicas. The next scale-up requires a cold start which may introduce additional latency.",
		...
		ctx.cases = append(ctx.cases, interpretationCase{
			reason:     "ScaleToZero",
			message:    confidence.BadgeObserved + " Scale-to-zero is enabled (minReplicas=0) and the HPA wants to scale to zero. Note: scaling from 0 back to 1 requires a cold start.",
