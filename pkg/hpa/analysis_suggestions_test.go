...
func TestBuildSuggestionsAddsBehaviorAndToleranceRecommendations(t *testing.T) {
	...
	if !containsSuggestion(got.Actions.Suggestions, "Add explicit scale-up policy") {
		t.Fatalf("expected scale-up policy suggestion, got %#v", got.Actions.Suggestions)
	}
	if !containsSuggestion(got.Actions.Suggestions, "Review scale-up tolerance") {
		t.Fatalf("expected tolerance suggestion, got %#v", got.Actions.Suggestions)
	}
}
