...
func registerStatusFlags(cmd *cobra.Command, opts *options) {
	cmd.Flags().BoolVar(&opts.features.interpret, "interpret", false, "include interpretation in status output")
	cmd.Flags().BoolVar(&opts.features.explain, "explain", false, "include detailed interpretation and recommended actions")
	cmd.Flags().BoolVar(&opts.features.suggest, "suggest", false, "include concrete suggestions for configuration changes")
	cmd.Flags().BoolVar(&opts.features.fix, "fix", false, "show stronger fix plan with patch commands")
	cmd.Flags().BoolVar(&opts.features.recommend, "recommend", false, "alias for --suggest")
	cmd.Flags().BoolVar(&opts.features.noInterpret, "no-interpret", false, "omit interpretation and show raw status-derived data")
	...
	cmd.Flags().BoolVar(&opts.features.diagnoseMetrics, "diagnose-metrics", false, "run comprehensive metrics pipeline health checks")
	cmd.Flags().BoolVar(&opts.features.metricsFreshness, "metrics-freshness", false, "analyze per-metric data freshness, source, and staleness risk")
	cmd.Flags().StringVar(&opts.vpa, "vpa", "auto", "VPA conflict detection: auto (enable when CRD present), on (force), off (disable)")
	cmd.Flags().BoolVar(&opts.features.checkResources, "check-resources", false, "check HPA target utilization against pod resource requests")
	cmd.Flags().BoolVar(&opts.features.explainPods, "explain-pods", false, "analyze scale target pods for readiness, resource requests, and metric coverage")
	...
	cmd.Flags().BoolVar(&opts.features.capacityContext, "capacity-context", false, "check infrastructure capacity constraints affecting HPA scaling")
	cmd.Flags().BoolVar(&opts.features.capacityHeadroom, "capacity-headroom", false, "estimate resource headroom needed to reach maxReplicas")
	cmd.Flags().BoolVar(&opts.features.scalePath, "scale-path", false, "explain the path from HPA desired replicas to pods and scheduler capacity")
	cmd.Flags().BoolVar(&opts.features.decisionTrace, "decision-trace", false, "show a step-by-step visible HPA decision trace")
	cmd.Flags().BoolVar(&opts.features.rollout, "rollout", false, "include rollout-aware workload diagnosis")
	cmd.Flags().BoolVar(&opts.features.rolloutImpact, "rollout-impact", false, "show how Deployment/StatefulSet rollout state affects HPA scale-out")
	cmd.Flags().BoolVar(&opts.features.readinessImpact, "readiness-impact", false, "show how not-yet-ready pods and missing PodMetrics may affect HPA decisions")
	cmd.Flags().BoolVar(&opts.features.scaleoutBlockers, "scaleout-blockers", false, "rank visible blockers preventing HPA scale-out from producing Ready pods")
	cmd.Flags().BoolVar(&opts.features.controllerProfile, "controller-profile", false, "show HPA controller-manager timing assumptions used for interpretation")
	...
	cmd.Flags().BoolVar(&opts.features.hiddenFactors, "hidden-factors", false, "show missing metrics, not-yet-ready pod, tolerance, and stabilization factors that are only partially visible")
	cmd.Flags().BoolVar(&opts.features.nodeAutoscaler, "node-autoscaler", false, "include Cluster Autoscaler scale-out context in status/doctor analysis")
	cmd.Flags().BoolVar(&opts.features.karpenter, "karpenter", false, "include Karpenter-style node provisioning context in status/doctor analysis")
	cmd.Flags().BoolVar(&opts.features.contextForAI, "context-for-ai", false, "emit a compact local-AI context pack instead of normal status text")
	cmd.Flags().StringVar(&opts.ask, "ask", "", "include a local-AI question in the context pack; no external LLM call is made")
	cmd.Flags().BoolVar(&opts.features.capacityDeep, "capacity-deep", false, "deep capacity analysis for scale-out blockers including node capacity and container failures")
	cmd.Flags().BoolVar(&opts.features.capacityPlan, "capacity-plan", false, "run capacity plan analysis when HPA is at maxReplicas")
	...
	cmd.Flags().BoolVar(&opts.features.gitopsCheck, "gitops-check", false, "detect GitOps manifest conflicts with HPA-managed replicas")
	cmd.Flags().StringVar(&opts.manifestPath, "manifest", "", "path to manifest file or directory for GitOps conflict detection")
	cmd.Flags().BoolVar(&opts.features.metricContract, "metric-contract", false, "verify HPA metric references are queryable from metrics APIs")
	cmd.Flags().BoolVar(&opts.features.churnDetect, "churn-detect", false, "detect replica thrashing and recommend stabilization adjustments")
	cmd.Flags().BoolVar(&opts.features.metricHints, "metric-hints", false, "troubleshoot custom/external metric issues with common failure pattern hints")
	cmd.Flags().BoolVar(&opts.features.containerAdvisor, "container-advisor", false, "suggest ContainerResource metrics for multi-container HPA targets")
	cmd.Flags().BoolVar(&opts.features.behaviorAdvisor, "behavior-advisor", false, "analyze behavior config and suggest stabilization/policy tuning")
	...
	cmd.Flags().BoolVar(&opts.features.flappingAdvisor, "flapping-advisor", false, "recommend stabilization window changes to reduce replica flapping")
	cmd.Flags().BoolVar(&opts.features.trendAnomaly, "trend-anomaly", false, "detect anomalies in health score history (enabled by default with --trend)")
	...
	cmd.Flags().BoolVar(&opts.features.adapterDiagnostics, "adapter-diagnostics", false, "diagnose custom/external metrics adapter signals")
}
