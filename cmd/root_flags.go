...
func registerStatusFlags(cmd *cobra.Command, opts *options) {
	...
	cmd.PersistentFlags().StringVar(&opts.format, "format", "", "status output profile: structured")
	...
	cmd.PersistentFlags().BoolVar(&opts.contextForAI, "context-for-ai", false, "emit a compact local-AI context pack instead of normal status text")
	cmd.PersistentFlags().StringVar(&opts.ask, "ask", "", "include a local-AI question in the context pack; no external LLM call is made")
