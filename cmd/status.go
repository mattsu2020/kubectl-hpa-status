...
func runStatusSingle(ctx context.Context, out io.Writer, opts *options, name string, includeInterpretation bool) error {
	...
	if opts.Format == "structured" {
		if report.Analysis.Decision.StructuredDecisionTrace == nil {
			report.Analysis.Decision.StructuredDecisionTrace = hpaanalysis.ExportStructuredDecisionTrace(nil, report.Analysis)
		}
		return joinOutputAndExit(
			render.Format(out, "json", "", report.Analysis.Decision.StructuredDecisionTrace, nil),
	...
	if opts.Apply {
		applied, err := applySuggestions(ctx, out, opts, name, report.Analysis.Actions.Suggestions)
		if err != nil {
			return err
		}
	...
	return warningExitCode(report.Analysis.Decision.Health, report.Analysis.Meta.Name, report.Analysis.Meta.Namespace, watchMode)
}
...
func renderStructuredDecisionTraces(out io.Writer, results []reportResult) error {
	...
	for i := range results {
		...
		tr := results[i].report.Analysis.Decision.StructuredDecisionTrace
		if tr == nil {
