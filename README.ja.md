...
| `--suggest`, `--recommend` | `status`, `analyze` | 安全に見える修正案を `kubectl patch` として表示。 |
...
| custom/external metrics adapters | HPA statusに見える範囲で対応。ratio解釈はbest-effortで、adapter内部状態は直接検査しません |
