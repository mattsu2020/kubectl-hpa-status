...
Metrics freshness出力では、adapter側の値欠落や古さを強調します。
...
| `--metrics-freshness` | `status`, `doctor`, `analyze` | HPA metricsの有無、stale状態、metrics API discovery可否、直近FailedGet*Metric Eventsとの関連を確認。`doctor` ではデフォルトで有効。 |
...
- [x] **Metrics freshness / staleness analyzer:** `--metrics-freshness` でHPA currentMetrics、FailedGet*Metric Events、metrics API discovery、PodMetrics timestamp/window、KEDA trigger文脈を確認。
...
- [ ] **Custom / External Metrics深掘り:** API discoveryとHPAから見えるfreshness signalを超え、adapter固有の推定やPrometheus/custom metricsの確認ヒントを追加。
