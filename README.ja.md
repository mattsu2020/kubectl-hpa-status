...
| `--apply` | `status`, `analyze`, `list`, `scan` | デフォルトでserver-side dry-runとしてHPA patchを検証。`list` では `--problem`, `--filter`, score filter のいずれかと組み合わせます。 |
...
| `--debug`, `-v` | `status`, `analyze`, `list` | metric ratio、health score、condition根拠など内部計算を表示。 |
| `--lang=ja`, `-o ja` | text出力 | 日本語ラベルで表示。 |
| `--no-interpret` | `status`, `analyze` | 解釈を省き、ステータス由来のデータのみ表示。 |
| `--events=false` | `status`, `analyze` | 最近のEventsを省略。 |
| `--events=3` | `status`, `analyze` | 最新3件のHPA Eventsを表示。 |
| `--diagnose-metrics` | `status`, `analyze` | メトリクス種別ごとの取得状況、adapter/APIService確認のヒント、次の確認手順を表示。 |
| `--check-resources` | `status`, `analyze` | HPA対象Podのresource request/limitとtarget utilizationの整合性を確認。 |
| `--report markdown\|html` | `status`, `list` | 単体またはクラスタ全体の診断レポートをMarkdown/HTMLで生成。 |
...
| すべてのメトリクスが `<unknown>` | `kubectl hpa status <name> --diagnose-metrics` | メトリクス別ヘルスチェック、status欠落 | metrics-server、custom metrics adapter登録、APIServiceの健全性を確認 |
| HPA target utilizationが期待と違う | `kubectl hpa status <name> --check-resources` | resource request警告、request未設定、target mismatch | Pod templateのresource requestsを確認。HPA utilizationはusage/requestで計算される |
...
**`Metrics unavailable` と表示されたら最初に何を実行すべきですか?** `kubectl hpa status <name> --explain --diagnose-metrics` から始めてください。CPU/メモリなら `kubectl top pods` とmetrics-serverを確認します。custom/external metricsならadapterの `APIService`、adapterログ、metric selectorの意味を確認します。
...
`timeline --since` で過去のスケーリング判断を推定表示:
